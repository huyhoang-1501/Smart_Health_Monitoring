...
void gui_va_doc_du_lieu()
{
  if (Firebase.ready() && signUp)
  {
    ...
    // Gửi dữ liệu lên RealtimeDatabase của Firebase
    ...
    Firebase.RTDB.setInt(&fbdo, "/parameter/steps", stepCount);
     if (Firebase.RTDB.getString(&fbdo, "/user/phone/sdt")) {
      ...
      {
        String sdt = fbdo.stringData();  // Đọc dữ liệu số điện thoại trên RealtimeDatabase
        Serial.print(" Số điện thoại đọc được từ Firebase: ");
        Serial.println(sdt);
        if (sdt.length() == 10) // Kiểm tra độ dài của sdt mới đọc từ Firebase
        {
          if(sdt != savedNumbers) // Kiểm tra sdt mới có giống với sdt cũ đã lưu
          {
            savedNumbers = sdt;
            Serial.print(" Đã lưu số điện thoại mới");
          }
          else Serial.print(" Giống số điện thoại đã lưu");
        }
      }
    }else 
    {
 ...
 if (bpm_warning >= 3 || step_warning >= 2)
  {
    ...
    if (warning_enable)
    {
     ...
     if (elapsed >= WARNING_DURATION)
     {
      ...
      delay(1000);
      sendAT("ATD" + savedNumbers + ";");
      warningStartTime = millis();  // Đọc thời gian để thực hiện lần gọi tiếp theo
     }
    }
  }
}
